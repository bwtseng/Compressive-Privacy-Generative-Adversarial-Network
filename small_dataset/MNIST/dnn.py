...
class DNN:
    ...
    def load_data(self):
        ...
        v_label = mnist.test.labels
        '''
        #t_data  = np.pad(t_data, ((0,0),(2,2),(2,2),(0,0)), 'constant')
        #t_label = np.pad(t_label, ((0,0),(2,2),(2,2),(0,0)), 'constant')
    ...
    def Alex_net(self, input, name_1, name_2):
        ...
        with tf.variable_scope(name_1):
            ...
            flat = ly.flatten(conv5) # Dimension 256.
            print(conv5)
    ...
    def adversary_krr(self, kernel_vec, name, reuse=False):
        ...
        with tf.variable_scope('adversary_krr_'+name) as scope:  
            ...
            recontruction = ly.fully_connected(kernel_vec, 28*28*1, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer(), biases_initializer = None)
    ...
    def DNN_with_resize(self):
        ...
        self.krr_weights_white_box_p = tf.placeholder(tf.float32, shape=[self.mapping_dim, 28*28*1])
        self.lrr_weights_white_box_p = tf.placeholder(tf.float32, shape=[self.com_dim, 28*28*1])
        
        self.krr_weights_pca_p = tf.placeholder(tf.float32, shape=[self.mapping_dim_pca, 28*28*1])
        self.lrr_weights_pca_p = tf.placeholder(tf.float32, shape=[self.pca_dim, 28*28*1])
        ...
        with tf.variable_scope("Decov"):
            ...
            latent = ly.fully_connected(self.com_input_p, 7*7*64, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            ...
            self.upsampling = ly.conv2d_transpose(upsampling, 1, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
       

        ## PCA Stage.

        with tf.variable_scope("Classifier_PCA"):
            #fs_pca = ly.fully_connected(self.perturbated_data, 120, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            fs_pca = ly.fully_connected(self.pca_input, 120, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            fs_pca = ly.fully_connected(fs_pca, 84, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            self.logit_pca = ly.fully_connected(fs_pca, 10, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer())
            self.prob_pca = tf.nn.softmax(self.logit_pca)

        with tf.variable_scope("Decoder_PCA"):
            #latent_pca = ly.fully_connected(self.perturbated_data, 7*7*64, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            latent_pca = ly.fully_connected(self.pca_input, 7*7*64, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            pre_reshape_pca = tf.reshape(latent_pca, shape=[-1, 7, 7, 64])
            upsampling_pca = ly.conv2d_transpose(pre_reshape_pca, 32, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            self.upsampling_pca = ly.conv2d_transpose(upsampling_pca, 1, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
    ...
    def train(self):
        ...
        for _ in range(40):
            for i, j, k in self.eva_next_batch(t_emb, self.t_data, self.t_label, self.batch_size, shuffle=False):
                ...
                #no  = np.random.normal(size=(sample, self.com_dim))
                no = np.random.laplace(0, self.noise_scale, size=(sample, self.com_dim))
                feed_dict = {} 
    ...
    def eva_next_batch(self, t_emb, t_data, t_label, batch_size, shuffle=False):
        ...
        for i in range(0, le, batch_size):
            ...
            if i ==  (epo *batch_size) : 
                yield np.array(t_emb[i:]), np.array(t_data[i:]).reshape(-1, 28, 28, 1) , np.array(t_label[i:])
            else : 
                yield np.array(t_emb[i: i+self.batch_size]), np.array(t_data[i: i+self.batch_size]).reshape(-1, 28, 28, 1) , np.array(t_label[i: i+self.batch_size])
