...
class wrs:
    def __init__(self,arg):
        ...
        print(len(self.te_data))
        print(len(self.te_label))
        ...
        with tf.control_dependencies(uti_update):
            ...
            #self.op = tf.train.MomentumOptimizer(0.01 , 0.9, use_nesterov=True)
        ...
        '''
        temp = np.load('normal_log/theta_g.npy')
        self.assign_op = [] 
        for i in range(len(temp)):
            self.assign_op.append(tf.assign(self.theta_c[i],temp[i]))
        '''
    ...
    '''
    def wideres33block(self, x, input_channel, output_channel, s, N, scope):
        ...
        equalInOut = (input_channel == output_channel)
        print(equalInOut)
        with tf.variable_scope(scope):
            if not equalInOut: 
                x = self.bo_batch_norm(x, self.is_train)
            else : 
                out = self.bo_batch_norm(x, self.is_train)

            if equalInOut : 
                conv33_1 = ly.conv2d(out, kernel_size=3, num_outputs=output_channel, padding='SAME', stride=s, activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer())
                conv33_1 = self.bo_batch_norm(conv33_1,self.is_train)
                conv33_1 = tf.nn.dropout(conv33_1,keep_prob = self.dropout)
            else :  
                conv33_1 = ly.conv2d(x, kernel_size=3, num_outputs=output_channel, padding='SAME', stride=s, activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer())
                conv33_1 = self.bo_batch_norm(conv33_1,self.is_train)
                conv33_1 = tf.nn.dropout(conv33_1,keep_prob = self.dropout)

            conv33_2 = ly.conv2d(conv33_1, kernel_size=3, num_outputs=output_channel, stride=1, activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer())        

            if equalInOut : 
                caddtable = tf.add(conv33_2,x)
            else : 
                conv_skip= ly.conv2d(x, kernel_size=1, num_outputs=output_channel, stride=s, padding='VALID', activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer()) #shortcut connection
                caddtable = tf.add(conv33_2,conv_skip)

            for i in range(0,N-1):        
                C = caddtable
                Cactivated = self.bo_batch_norm(C,self.is_train)   
                conv33_1 = ly.conv2d(Cactivated,kernel_size=3,num_outputs=output_channel,stride=1,activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer())
                conv33_1 = self.bo_batch_norm(conv33_1,self.is_train)
                conv33_1 = tf.nn.dropout(conv33_1,self.dropout)
                conv33_2 = ly.conv2d(conv33_1,kernel_size=3,num_outputs=output_channel,stride=1,activation_fn=None, biases_initializer=None, weights_initializer=tf.contrib.layers.xavier_initializer())
                caddtable = tf.add(conv33_2,C)
    ...
    def train(self):
        ...
        for epo in range(epochs) : 
            for i , j , q , k , aug in self.t_next_batch(self.t_data,self.t_label):
                ...
                #print(self.sess.run(self.x,feed_dict=feed_dict))
            ...
            if epo %10 == 0:
                self.saver.save(self.sess,'svhn_log/model_'+str(epo+1))
