...
class hybrid_CPGAN:
    ...
    def __init__(self, arg):
        self.arg = arg
        self.ori_dim = 64 * 64 *3 
        self.com_dim = self.arg.com_dim #200 #3136 #1024
    ...
    def adversary_nn(self, final_latent, reuse=False):
        ...
        with tf.variable_scope('adversary_nn') as scope:
            if reuse:
                scope.reuse_variables()
            final_latent = ly.fully_connected(final_latent, 4*4*256, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            reshape_decov = tf.reshape(final_latent, shape=[-1, 4, 4, 256])
            upsampling = ly.conv2d_transpose(reshape_decov, 128, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            ...
            upsampling = ly.conv2d_transpose(upsampling, 3, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
        return upsampling
    ...
    def CPGAN(self): 
        ...
        with tf.variable_scope('white_box'):
            ...
            latent_white_box = ly.fully_connected(self.emb_p, 4*4*256, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            latent_white_box = tf.reshape(latent_white_box, shape=[-1, 4, 4, 256])
            ...
            upsampling_white_box = ly.conv2d_transpose(latent_white_box, 128, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            upsampling_white_box = ly.conv2d_transpose(upsampling_white_box, 64, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())     
            upsampling_white_box = ly.conv2d_transpose(upsampling_white_box, 32, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            self.recon_white_box = ly.conv2d_transpose(upsampling_white_box, 3, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
    ...
    def prediction_and_accuracy(self, data, label):
        ...
        print("Utility Accuracy (TPN): {}".format(acc))
    ...
    def evalute_privacy(self, data, v_data, label):
        ...
        imsave("/home/pywu/bowei/Genki/reconstruction/cpgan_1.png", self.plot(predict[0]))
        imsave("/home/pywu/bowei/Genki/reconstruction/cpgan_2.png", self.plot(predict[1]))
        ...
        for _ in range(self.epo):
            ...
            for i, j in self.next_batch(self.t_data, self.t_label, self.batch_size, shuffle=True):
                ...
                for _ in range(self.citer):
                    ...
                    #grad_c, grad_d = self.sess.run([self.gradient_norm_c, self.gradient_norm_d], feed_dict=feed_dict)
            ...
            epo += 1 
        np.save("grad_c_cpgan.npy", grad_c_record)
        np.save("grad_d_cpgan.npy", grad_d_record)
