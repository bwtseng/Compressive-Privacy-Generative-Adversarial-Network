...
class RAN:
    ...
    def LeNet(self, input, name_1, name_2):
        ...
        with tf.variable_scope(name_1):
            ...
            #flat = ly.fully_connected(flat, 512, activation_fn=None)
    ...
    def RAN(self):
        ...
        self.krr_weights_p = tf.placeholder(tf.float32, shape=[self.mapping_dim, 64*64*3])
        self.lrr_weights_p = tf.placeholder(tf.float32, shape=[self.com_dim, 64*64*3])
        ...
        with tf.variable_scope('white_box'):
            #reshape_decov = tf.reshape(self.emb_p, shape=[-1, 4, 4, 64])
            latent = ly.fully_connected(self.emb_p, 4*4*256, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            pre_reshape = tf.reshape(latent, shape=[-1, 4, 4, 256])
            upsampling = ly.conv2d_transpose(pre_reshape, 128, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            ...
            self.recon_white_box = ly.conv2d_transpose(upsampling, 3, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.variable_scope("Decoder"):
            #reshape = tf.reshape(self.compressing, shape=[-1, 4, 4, 64])
            latent_dec = ly.fully_connected(self.compressing, 4*4*256, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            reshape = tf.reshape(latent_dec, shape=[-1, 4, 4, 256])
            upsampling_dec = ly.conv2d_transpose(reshape, 128, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            upsampling_dec = ly.conv2d_transpose(upsampling_dec, 64, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())     
            upsampling_dec = ly.conv2d_transpose(upsampling_dec, 32, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
            self.upsampling = ly.conv2d_transpose(upsampling_dec, 3, kernel_size=3, stride=2, padding='SAME', activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer())
        ...
        #self.gradient_norm_cls = tf.reduce_mean(gradient_norm)
    ...
    def train(self):
        ...
        for _ in range(self.epo):
            ...
            for i, j in self.next_batch(self.t_data, self.t_label, self.batch_size, shuffle=True):
                ...
                #grad_c, gred_d = self.sess.run([self.gradient_norm_c, self.gradient_norm_d], feed_dict=feed_dict)
                #_ = self.sess.run([self.e_c_opt], feed_dict=feed_dict)

                _, grad_c, grad_d = self.sess.run([self.g_opt, self.gradient_norm_c, self.gradient_norm_d], feed_dict=feed_dict)
                grad_c_record.append(grad_c)
                grad_d_record.append(grad_d)
                print(grad_c, grad_d)

            epo +=1
        np.save("grad_c.npy", grad_c_record)
        np.save("grad_d.npy", grad_d_record)
    ...
    def evalute_privacy(self, data, v_data, label):
        ...
        imsave("/home/pywu/bowei/Genki/reconstruction/ran_1.png", self.plot(predict[0]))
        imsave("/home/pywu/bowei/Genki/reconstruction/ran_2.png", self.plot(predict[1]))
